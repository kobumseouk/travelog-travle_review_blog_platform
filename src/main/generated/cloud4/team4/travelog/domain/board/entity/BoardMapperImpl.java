package cloud4.team4.travelog.domain.board.entity;

import cloud4.team4.travelog.domain.board.dto.BoardCreateRequestDto;
import cloud4.team4.travelog.domain.board.dto.BoardUpdateRequestDto;
import javax.annotation.processing.Generated;
import org.springframework.stereotype.Component;

@Generated(
    value = "org.mapstruct.ap.MappingProcessor",
    date = "2024-08-22T18:07:24+0900",
    comments = "version: 1.5.3.Final, compiler: javac, environment: Java 22.0.1 (Oracle Corporation)"
)
@Component
public class BoardMapperImpl implements BoardMapper {

    @Override
    public Board toCreateEntity(BoardCreateRequestDto dto) {
        if ( dto == null ) {
            return null;
        }

        Board.BoardBuilder board = Board.builder();

        board.regionMajor( dto.getRegionMajor() );
        board.regionMiddle( dto.getRegionMiddle() );
        board.description( dto.getDescription() );

        return board.build();
    }

    @Override
    public BoardUpdateRequestDto toBoardUpdateRequestDto(Board board) {
        if ( board == null ) {
            return null;
        }

        Board board1 = null;

        board1 = board;

        BoardUpdateRequestDto boardUpdateRequestDto = new BoardUpdateRequestDto( board1 );

        return boardUpdateRequestDto;
    }

    @Override
    public Board toUpdateEntity(BoardUpdateRequestDto boardUpdateRequestDto) {
        if ( boardUpdateRequestDto == null ) {
            return null;
        }

        Board.BoardBuilder board = Board.builder();

        board.description( boardUpdateRequestDto.getDescription() );

        return board.build();
    }
}
